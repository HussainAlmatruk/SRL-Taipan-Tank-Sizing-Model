...
P.t_burn_s      = 7.8;                   % [s]        Total burn time
...
P.p_storage_pressurant_pa = 4500 * C.PSI_TO_PA;    % [Pa] The pressure the Nitrogen is stored in dedicated tank (MEOP)
...
% Standard conditions for gas flow calculations (SCFM)
P.standard_temperature_k  = 20 + C.C_TO_K_OFFSET;  % [K] Standard temperature for volumetric flow (20 C)
P.standard_pressure_pa    = 101325;                % [Pa] Standard pressure for volumetric flow (1 atm)
...
%
% Current tank: SCOTT 45 Min. 4500 PSI Cylinder (PN 804722-01)
% Specs from spec sheet:
%   Height:         23.40 in  (0.5944 m) — includes valve
%   Outer Diameter: 6.26 in   (0.1590 m) 
%   Weight:         14.99 lbs (6.80 kg) 
%   Water Volume:   6.8 L     (0.0068 m^3)
%
...
P.cots_tank_outer_diameter_m  = 6.26 * C.IN_TO_M;       % [m]   Outer diameter (6.26 in)
P.cots_tank_length_m          = 23.40 * C.IN_TO_M;      % [m]   Total height including valve (23.40 in)

% --- ESTIMATED dimensions ---
% For a 4500 PSI composite cylinder, combined thickness (liner + wrap)
% is typically around ~0.45 inches (~12 mm). If OD = 6.26, and Inner
% Vol is 418 cu in, an inner diam of ~5.3 in leaves exactly this much room.
P.cots_tank_wall_thickness_m  = 0.012;                                                       % %%% ESTIMATED %%% [m] Wall thickness (~12 mm)
...
% gas.  The total spec-sheet mass includes the valve (~0.6 kg).  We
% subtract the valve mass to get the wall mass alone.
P.cots_tank_valve_mass_kg = 0.6;                                                       % %%% ESTIMATED %%% [kg] Approximate mass of the cylinder valve
...
P.ullage_fraction_fuel  = 0.001;     % %%% TEMPORARY VALUE %%%  [unitless] Percent of empty volume in tanks (e.g., 0.1 for 10%)
